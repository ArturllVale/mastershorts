...
export default function ViralHUD({ score, explanation, reasons = [], risks = [] }) {
    ...
    return (
        <div className="mb-3 bg-paper/80 rounded-input border border-rule p-2.5 sm:p-3 flex gap-2.5 sm:gap-3 items-center shadow-sm min-w-0 relative">
            <div className="relative group/score shrink-0">
                <div className="flex flex-col items-center justify-center bg-paper2 border border-rule2 rounded-input w-12 h-12 py-1 px-1 relative cursor-pointer hover:border-ok/50 transition-colors">
                    <span className={
                ...
                {hasDetails && (
                    <div className="absolute left-0 top-full mt-2 w-72 bg-paper2 border border-rule2 shadow-xl shadow-black/20 rounded-xl p-4 z-[100] opacity-0 invisible group-hover/score:opacity-100 group-hover/score:visible transition-all duration-200 pointer-events-none group-hover/score:pointer-events-auto group-hover/score:translate-y-0 -translate-y-1">
                        {reasons?.length > 0 && (
