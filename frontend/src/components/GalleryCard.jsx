...
export default function GalleryCard({ clip }) {
    ...
    return (
        ...
        >
            ...
            <div className="aspect-[9/16] bg-surface-1 relative group/video">
                {isVisible ? (
                    <video
                        ref={videoRef}
                        src={clip.url}
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-surface-1">
                        <div className="w-12 h-12 rounded-full bg-surface-2 border border-border flex items-center justify-center">
                            <Play size={22} className="text-text-tertiary ml-0.5" />
                        </div>
                    </div>
                )}
                <div className="absolute top-2 left-2">
                    <span className="font-mono text-[10px] bg-surface-1/90 backdrop-blur-md border border-border text-text-secondary px-2 py-0.5 rounded-full">
            ...
            <div className="flex-1 p-3.5 flex flex-col min-w-0">
                <div className="mb-3">
                    <h3 className="text-xs font-medium text-text-primary leading-tight line-clamp-2 mb-2 break-words" title={clip.title}>
                        {clip.title}
                    </h3>
                    <div className="flex flex-wrap gap-1.5">
                        <span className="font-mono text-[10px] bg-surface-2 text-text-tertiary px-1.5 py-0.5 rounded">{clip.duration.toFixed(1)}s</span>
                        <span className="font-mono text-[10px] bg-surface-2 text-text-tertiary px-1.5 py-0.5 rounded truncate max-w-[150px]" title={clip.job_id}>ID: {clip.job_id.substring(0, 8)}</span>
                    </div>
                </div>
