...
const FEATURES = [
  ...
  ['REST API for automations', 'Automate clipping end to end with a REST API, per-user keys and completion webhooks.'],
...
const FAQ = [
  [
    'Is MasterShorts free? What is the catch?',
    CANONICAL_ANSWERS.isItFree +
      ' Self-hosted costs you hardware and time: on a typical CPU an 8-minute video takes 5 to 8 minutes to process, and you supply your own Google Gemini key, whose free tier covers 1,500 requests a day. On MasterShorts Cloud an NVIDIA GPU clips that same video in about 50 seconds, the Gemini key is included, auto-posting to TikTok, Instagram and YouTube is already wired up, and your clips stay retrievable from any browser.',
  ],
  ['What is MasterShorts and how does it work?', CANONICAL_ANSWERS.whatIsIt + ' ' + CANONICAL_ANSWERS.howItWorks],
  [
    'How does MasterShorts compare to Opus Clip?',
    'Both do AI viral moment detection and smart vertical cropping. MasterShorts is MIT-licensed and can be self-hosted, so the source video never leaves your machine, and it adds voice dubbing into 30+ languages plus an AI UGC generator with lip-synced actors. Opus Clip is closed source and cloud only, starting at $15/month as of July 2026, and it ships a larger caption-style library. Full comparison at /alternatives/opus-clip.',
  ],
  [
  ...
  [
    'Can MasterShorts translate and dub videos?',
  ...
  [
    'Can I automate MasterShorts with the API?',
    'Yes. MasterShorts provides a REST API with per-user keys and completion webhooks, so your scripts or workflows can submit a video URL, receive finished clips, and publish them to social platforms automatically.',
  ],
  [
...
<section style="${S.section};padding-top:4rem"><div style="${S.wrap}">
  ...
  <p style="${S.p};max-width:44rem;font-size:1.05rem">Turn long videos into viral 9:16 shorts, or generate UGC marketing videos with AI actors. Online in the cloud with zero setup, or self-hosted locally for free.</p>
  <p style="${S.muted};max-width:44rem"><strong style="color:oklch(75% 0.11 150)">No credit card required.</strong> 20 free minutes every month. Paid plans from $12/month without a watermark. Prefer to run it yourself? <a style="${S.a}" href="https://github.com/ArturllVale/mastershorts" rel="noopener">Self-host free on GitHub</a>.</p>
</div></section>

<section style="${S.section}"><div style="${S.wrap}">
  <h2 style="${S.h2}">what mastershorts is</h2>
...
<section style="${S.section}"><div style="${S.wrap}">
  ...
  <div style="${S.grid};max-width:56rem">
    ${card('MasterShorts self-hosted &middot; $0', EDITIONS.selfHosted.summary)}
    ${card('MasterShorts Cloud &middot; free tier, then from $12/month', EDITIONS.cloud.summary)}
  </div>
</div></section>
...
<section style="${S.section}"><div style="${S.wrap}">
  ...
  <p style="${S.muted};max-width:48rem">Entry pricing checked 27 July 2026. MasterShorts is $0 self-hosted or $12/month hosted without a watermark. Submagic starts at $14/month, Opus Clip at $15/month, Vizard at $19.99/month and Klap at $29/month. MasterShorts is the only open source and self-hostable option of the five.</p>
  <p style="${S.muted}">
    ...
    <a style="${S.a}" href="/how-mastershorts-works">How it works</a> &middot;
