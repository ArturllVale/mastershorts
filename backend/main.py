    ...
    def get_crop_box(self, force_snap=False):
        ...
        if force_snap:
            ...
            if abs(diff) > self.safe_zone_radius:
                # Se a mudança de distância for muito grande (ex: mudou quem fala),
                # força um CORTE SECO (Hard Cut) ao invés de arrastar a câmera rápido.
...
def sanitize_filename(filename):
    ...
    # Strip emojis and 4-byte UTF-8 non-BMP characters (like 🔴) which cause 'Invalid argument' in Windows tools
...
def download_youtube_video(url, output_dir="."):
    ...
    def _base_opts(extractor_args, proxy, cookies=True):
        return {
            ...
            'noplaylist': True,
            'nopart': True,  # Prevent .part renaming which Windows Defender/indexer locks
            'continuedl': False,  # Prevent resuming broken/expired byte ranges which triggers HTTP 416
            'file_access_retries': 15,  # Retry file access/renames up to 15 times on Windows
            'retry_sleep_functions': {'file_access': lambda n: 0.5 * (2 ** min(n, 3))},
            'windowsfilenames': True,  # Force Windows-safe file names
    ...
    def _attempt(extractor_args, fmt, proxy, cookies=True):
        ...
        # Clean up any previous complete or partial files for this title to prevent 416 Range errors
        try:
