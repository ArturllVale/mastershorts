...
def _transcribe_with_whisper(media_path):
    ...
    # If VAD filter eliminated all segments on media with audio, retry without VAD
    if not out_segments and WHISPER_TRANSCRIBE_PARAMS.get("vad_filter", True):
        print("⚠️ [ASR] Whisper VAD filter produced 0 segments — retrying without VAD filter...", flush=True)
        no_vad_params = dict(WHISPER_TRANSCRIBE_PARAMS)
        no_vad_params["vad_filter"] = False
        try:
            segments, info = run_whisper_transcription(media_path, **no_vad_params)
            for segment in segments:
                words = [
                    {"word": w.word, "start": float(w.start), "end": float(w.end)}
                    for w in (segment.words or [])
                ...
                text_parts.append(segment.text.strip())
            if out_segments:
                print(f"✅ [ASR] Recovery succeeded without VAD: detected {len(out_segments)} segments in '{getattr(info, 'language', 'unknown')}'", flush=True)
        except Exception as retry_err:
            print(f"⚠️ [ASR] Whisper fallback without VAD also encountered error: {retry_err}", flush=True)
...
def transcribe_media(media_path):
    ...
    if not _has_audio_stream(media_path):
        raise NoAudioError(
            "This video has no audio track. MasterShorts finds viral moments from "
