...
download = pytest.importorskip("download")
download = pytest.importorskip("download")          # cv2 etc., absent in minimal CI
...
class TestTruncateBytes:
    def test_short_text_untouched(self):
        assert download.truncate_bytes("hello", 100) == "hello"

    def test_trims_to_the_byte_budget(self):
        out = download.truncate_bytes(BENGALI * 5, 120)
    ...
    def test_never_splits_a_multibyte_character(self):
        ...
        for budget in range(1, 40):
            out = download.truncate_bytes(BENGALI, budget)
    ...
    def test_ascii_is_unaffected_by_the_byte_rule(self):
        text = "a" * 200
        assert download.truncate_bytes(text, 120) == "a" * 120


class TestSanitizeFilename:
    def test_non_latin_title_fits_the_budget(self):
        out = download.sanitize_filename(BENGALI * 4)
        assert len(out.encode("utf-8")) <= download.MAX_TITLE_BYTES
    ...
    def test_leaves_room_for_every_decoration_the_pipeline_adds(self):
        ...
        stem = download.sanitize_filename(BENGALI * 4)
    ...
    def test_still_strips_unsafe_characters(self):
        assert download.sanitize_filename('a<b>c:d"e/f\\g|h?i*j#k') == "abcdefghijk"
        assert download.sanitize_filename("with spaces") == "with_spaces"
    ...
    def test_combining_accents_are_precomposed(self):
        ...
        assert download.sanitize_filename(decomposed) == precomposed
        assert download.sanitize_filename(precomposed) == precomposed
        assert "\u0301" not in download.sanitize_filename("Ha\u0301bitos_Nº1")
