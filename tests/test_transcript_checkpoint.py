...
output directory (resume manifest). checkpointing.py leaves the finished transcript
...
checkpointing = pytest.importorskip("checkpointing")  # needs cv2/mediapipe, absent in minimal CI
...
def _save(d, transcript=TRANSCRIPT, src=SRC, dur=DUR):
    checkpointing.save_transcript_checkpoint(str(d), transcript, src, dur)


def _load(d, src=SRC, dur=DUR):
    return checkpointing.load_transcript_checkpoint(str(d), src, dur)


class TestRoundTrip:
    def test_same_source_is_reused(self, tmp_path):
        _save(tmp_path)
        assert os.path.isfile(tmp_path / checkpointing.TRANSCRIPT_CHECKPOINT)
...
class TestNeverTheWrongTranscript:
    ...
    def test_legacy_shape_without_source_is_ignored(self, tmp_path):
        (tmp_path / checkpointing.TRANSCRIPT_CHECKPOINT).write_text(json.dumps(TRANSCRIPT))
        assert _load(tmp_path) is None
...
class TestRobustness:
    ...
    def test_truncated_file_is_ignored(self, tmp_path):
        (tmp_path / checkpointing.TRANSCRIPT_CHECKPOINT).write_text('{"source": {"name": "vi')
        assert _load(tmp_path) is None

    def test_unwritable_directory_does_not_raise(self, tmp_path):
        checkpointing.save_transcript_checkpoint(str(tmp_path / "missing" / "dir"), TRANSCRIPT, SRC, DUR)

    def test_clear_is_idempotent(self, tmp_path):
        _save(tmp_path)
        checkpointing.clear_transcript_checkpoint(str(tmp_path))
        checkpointing.clear_transcript_checkpoint(str(tmp_path))
        assert _load(tmp_path) is None
    ...
    def test_checkpoint_is_hidden_from_job_listings(self):
        ...
        assert checkpointing.TRANSCRIPT_CHECKPOINT.startswith(".")
        assert not checkpointing.TRANSCRIPT_CHECKPOINT.endswith("_metadata.json")
        assert not checkpointing.TRANSCRIPT_CHECKPOINT.endswith(".mp4")
