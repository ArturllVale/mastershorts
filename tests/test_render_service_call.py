...
@pytest.mark.asyncio
async def test_call_render_service_success(mock_httpx_client):
...
@pytest.mark.asyncio
async def test_call_render_service_unavailable(mock_httpx_client):
    mock_client_instance = mock_httpx_client.return_value.__aenter__.return_value
...
@pytest.mark.asyncio
async def test_call_render_service_invalid_json(mock_httpx_client):
    mock_post_res = MagicMock()
...
@pytest.mark.asyncio
async def test_call_render_service_render_error(mock_httpx_client):
    mock_post_res = MagicMock()
    mock_post_res.json.return_value = {"renderId": "123-abc"}
    mock_post_res.raise_for_status = MagicMock()
