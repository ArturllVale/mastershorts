...
viral_analysis = pytest.importorskip("viral_analysis")  # needs cv2/mediapipe, absent in minimal CI
...
class TestRealFailures:
    def test_nursery_rhyme_uh_uh_is_sparse(self):
        assert viral_analysis.speech_is_sparse(_t("Uh uh"), 169)

    def test_dashcam_yeah_is_sparse(self):
        assert viral_analysis.speech_is_sparse(_t("Yeah."), 621)
...
class TestRealSpeechIsKept:
    def test_a_talk_is_not_sparse(self):
        ...
        assert not viral_analysis.speech_is_sparse(_t(*words), 600)

    def test_a_short_clip_with_a_sentence_is_not_sparse(self):
        # 20s with one real sentence: nothing to gain from vision.
        assert not viral_analysis.speech_is_sparse(_t("welcome back everyone to another episode of the show"), 20)

    def test_a_song_with_lyrics_is_not_sparse(self):
        # "Wheels on the Bus" style: repetitive but plenty of words.
        assert not viral_analysis.speech_is_sparse(_t(*["the wheels on the bus go round and round"] * 30), 180)


class TestEdges:
    def test_empty_segments_is_sparse(self):
        assert viral_analysis.speech_is_sparse({"segments": []}, 100)

    def test_none_transcript_is_sparse(self):
        assert viral_analysis.speech_is_sparse(None, 100)

    def test_zero_duration_does_not_divide_by_zero(self):
        assert viral_analysis.speech_is_sparse(_t("hi"), 0)
        assert not viral_analysis.speech_is_sparse(_t(*["a b c d e f g h i j"] * 3), 0)
